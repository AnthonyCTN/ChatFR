...
if (isset($_GET['fetch_messages'])) {
    ...
    $messages = [];
    $sql = "SELECT m.contenu, m.timestamp, u.pseudo as expediteur, m.id_expediteur FROM messages m JOIN utilisateurs u ON m.id_expediteur = u.id WHERE (m.id_destinataire = ? AND m.id_expediteur = ?) OR (m.id_destinataire = ? AND m.id_expediteur = ?) ORDER BY m.timestamp DESC";
    $stmt = $conn->prepare($sql);
...
<body>
    ...
    <div class="chat-container">
        <div class="chat-header">
            ...
            <button class="view-profile" data-ami-id="<?= $ami_id ?>">Voir profil</button>
        </div>
...
.chat-messages {
    ...
    border-radius: 8px;
    padding: 10px;
    max-height: 300px; /* Limit the height to show a few messages at a time */
    display: flex;
    flex-direction: column-reverse; /* This makes the newest messages appear at the bottom */
}
...
/* Media queries pour ajuster la taille du bouton sur les petits écrans */
@media (max-width: 768px) {
    .btn-back {
        padding: 8px 16px;
        font-size: 12px;
