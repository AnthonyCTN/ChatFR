...
$stmt->bind_param("ii", $user_id, $user_id); // Liaison des paramètres
$stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['biographie'])) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("si", $biographie, $user_id); // Liaison des paramètres
        if ($stmt->execute()) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $user_id); // Liaison des paramètres
        $stmt->execute();
        $stmt->bind_result($hashed_password); // Récupération du mot de passe haché
        $stmt->fetch();
        $stmt->close();
        // Vérification si le mot de passe actuel est correct
        if (password_verify($old_password, $hashed_password)) {
            $new_hashed_password = password_hash($new_password, PASSWORD_DEFAULT); // Hachage du nouveau mot de passe
            // Mise à jour du mot de passe de l'utilisateur
            $sql = "UPDATE utilisateurs SET mot_de_passe = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $new_hashed_password, $user_id); // Liaison des paramètres
            if ($stmt->execute()) {
