...
<?php
    ...
    // Variables pour gérer les messages d'information ou d'erreur.
    $message = '';
    ...
    // Traitement du formulaire si une requête POST est envoyée et le champ 'ami_pseudo' est présent.
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ami_pseudo'])) {
        $ami_pseudo = $_POST['ami_pseudo'];

        // Recherche l'ID de l'utilisateur basé sur le pseudo fourni.
        ...
        if ($result->num_rows > 0) {
            ...
            // Vérification si une relation d'amitié existe déjà entre les deux utilisateurs.
            ...
            $result = $stmt->get_result();
            if ($result->num_rows == 0) {
                // Création de la relation d'amitié si elle n'existe pas.
